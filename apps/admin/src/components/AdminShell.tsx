  ...
  return <div className="admin-layout">
    ...
    <aside className={`admin-sidebar${menuOpen ? ' is-open' : ''}`}>
      ...
      <nav aria-label="Admin navigation">
        ...
        {futureNavigation.length>0&&<><p className="admin-nav-label admin-nav-future-label">SYSTEM MODULES</p><ul>{futureNavigation.map((label) => <li key={label}><span className="admin-nav-link is-disabled" aria-disabled="true"><span aria-hidden="true">—</span>{label}<small>Later</small></span></li>)}</ul></>}
      </nav>
    ...
    <div className="admin-workspace">
      <header className="admin-topbar"><button type="button" className="admin-menu-button" aria-label="Open navigation" aria-expanded={menuOpen} onClick={() => setMenuOpen(true)}><span /><span /><span /></button><div><span className="admin-topbar-label">INTERNAL SYSTEM</span><strong>Operations Control</strong></div><div className="admin-system-status"><span /> {adminDataMode==='firestore'?'FIREBASE DATA':'MOCK DATA'}</div></header>
      <Outlet />
    </div>
  </div>;
}
