...
export function OperationsPage() {
  ...
  return <main className="admin-main">
    ...
    <div className="results-summary"><span>{loading ? 'Loading operations' : `${filtered.length} of ${operations.length} operations`}</span>{hasFilters && <button type="button" onClick={() => { setSearch(''); setStatus('ALL'); setPackageType('ALL'); }}>Clear filters</button>}</div>
    {loading ? <div className="repository-state panel-state" aria-live="polite">Loading operations…</div> : <OperationList operations={filtered} />}
  </main>;
}
