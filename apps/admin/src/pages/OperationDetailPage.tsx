...
export function OperationDetailPage() {
  ...
  if (loadError) return <main className="admin-main"><div className="admin-notice is-error" role="alert">{loadError}</div><Link className="back-link" to="/operations">← Return to operations</Link></main>;
  if (!operation) return <main className="admin-main"><p className="eyebrow">OPERATION FILE</p><h1>Operation not found</h1><Link to="/operations">Return to operations</Link></main>;
  ...
  return <main className="admin-main operation-detail-page">
    <Link className="back-link" to="/operations">← Back to operations</Link>
    ...
    {action && <OperationActionDialog action={action} operationId={operation.operationId} onCancel={() => setAction(null)} onConfirm={confirm} />}
  </main>;
}
