...
const archiveEligibleStatuses = ['COMPLETED', 'CANCELLED', 'REFUNDED', 'REJECTED'];
export const setCustomerOperationArchived = onCall(async (request) => {
    const actor = requireAuthenticatedCustomer(request);
    try {
        const operationId = request.data.operationId?.trim();
        if (!operationId || typeof request.data.archived !== 'boolean')
            throw new HttpsError('invalid-argument', 'Operation ID and archive preference are required.');
        ...
        await db.runTransaction(async (transaction) => {
            ...
            if (!operationSnapshot.exists || !projectionSnapshot.exists)
                throw new HttpsError('not-found', 'Operation not found.');
            const operation = operationSnapshot.data();
            const projection = projectionSnapshot.data();
            if (operation.customerId !== actor.uid || projection?.customerId !== actor.uid)
                throw new HttpsError('permission-denied', 'You cannot change this operation.');
            if (archived && !archiveEligibleStatuses.includes(operation.status))
                throw new HttpsError('failed-precondition', 'Only completed or permanently closed operations can be archived.');
