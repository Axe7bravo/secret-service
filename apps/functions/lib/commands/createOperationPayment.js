...
export const createOperationPayment = onCall({ region: 'us-central1', secrets: [yocoSecretKey] }, async (request) => {
    ...
    try {
        ...
        if (!mode)
            throw new YocoProviderConfigurationError('The checkout secret has an unsupported mode.');
        ...
        const eligibility = await db.runTransaction(async (transaction) => {
            ...
            if (operation.package.currency !== 'ZAR' || !Number.isSafeInteger(operation.package.priceMinor) || operation.package.priceMinor <= 0)
            ...
            if (existing)
                validatePaymentOwnership(existing, operation);
            ...
            if (existing?.status === 'CANCELLED')
                throw new HttpsError('failed-precondition', 'This payment requires operator review.');
            const previousMode = existing?.checkoutRequest?.mode ?? existing?.processingMode?.toLowerCase();
            if (existing && !previousMode)
                throw new HttpsError('failed-precondition', 'The existing payment mode needs operator verification before retrying.');
            if (previousMode && previousMode !== mode)
                throw new HttpsError('failed-precondition', 'A payment cannot switch between test and live checkout.');
            ...
            if (existing?.status === 'PENDING' && existing.checkoutUrl) {
                if (!existing.providerCheckoutId)
                    throw new HttpsError('failed-precondition', 'The existing checkout reference needs operator review.');
            ...
            if (existing && !rotate && !existing.idempotencyKey)
                throw new HttpsError('failed-precondition', 'The existing checkout reservation needs operator review.');
            if (attempts.length >= MAX_PAYMENT_ATTEMPTS)
                throw new HttpsError('resource-exhausted', 'Payment checkout history is full. Contact support before retrying.');
            ...
            const checkoutRequest = !rotate && existing?.checkoutRequest ? existing.checkoutRequest : { ...configuredReturnUrls, mode };
            const reservation = { ...existing, paymentId: operationId, operationId, customerId: actor.uid, provider: 'YOCO', amountMinor: operation.package.priceMinor, currency: 'ZAR', status: 'PENDING', idempotencyKey, attemptNumber, checkoutRequest, ...paymentHistoryFields(attempts), createdAt: existing?.createdAt ?? now, updatedAt: now };
            if (rotate) {
                delete reservation.providerCheckoutId;
                delete reservation.providerPaymentId;
                delete reservation.checkoutUrl;
                delete reservation.failedAt;
                delete reservation.failureCategory;
            }
            transaction.set(paymentRef, reservation);
            return { operation, payment: reservation, idempotencyKey, checkoutRequest, checkoutReady: false };
        });
        ...
        try {
            ...
            await db.runTransaction(async (transaction) => {
                ...
                if (current.status === 'PENDING' && current.idempotencyKey === eligibility.idempotencyKey && !current.providerCheckoutId)
                    transaction.update(paymentRef, { failureCategory: error instanceof YocoProviderConfigurationError ? 'PROVIDER_NOT_CONFIGURED' : 'PROVIDER_INITIATION_FAILED', updatedAt: now });
            });
        ...
        const storedCheckout = await db.runTransaction(async (transaction) => {
            ...
            if (operation.customerId !== actor.uid)
                throw new HttpsError('permission-denied', 'Operation ownership changed.');
            if (!currentPayment.exists)
                throw new HttpsError('failed-precondition', 'Payment reservation is missing.');
            ...
            if (!known) {
                if (current.idempotencyKey !== eligibility.idempotencyKey)
                    throw new HttpsError('aborted', 'Checkout reservation changed. Operator reconciliation is required.');
                if (current.status === 'PENDING' && current.providerCheckoutId)
                    throw new HttpsError('failed-precondition', 'The provider returned conflicting checkout references for one reservation.');
                if (attempts.length >= MAX_PAYMENT_ATTEMPTS)
                    throw new HttpsError('resource-exhausted', 'Payment checkout history is full.');
                ...
                const payment = { ...current, ...paymentHistoryFields(attempts), updatedAt: now };
                if (current.status === 'PENDING') {
                    payment.providerCheckoutId = checkout.providerCheckoutId;
                    payment.checkoutUrl = checkout.checkoutUrl;
                    if (checkout.processingMode)
                        payment.processingMode = checkout.processingMode;
            ...
            if (current.status !== 'PENDING' || operation.status !== 'PAYMENT_PENDING' || operation.paymentSummary.status !== 'PENDING')
                return undefined;
            if (known && current.providerCheckoutId !== checkout.providerCheckoutId)
                return undefined;
            return { paymentId, checkoutUrl: checkout.checkoutUrl };
        });
        if (!storedCheckout)
            throw new HttpsError('failed-precondition', 'Payment eligibility changed. Refresh the operation before continuing.');
