...
export const createOperationPayment = onCall(async (request) => {
    ...
    try {
        ...
        const eligibility = await db.runTransaction(async (transaction) => {
            ...
            if (existing?.status === 'PENDING')
                return { operation, payment: existing, reuse: true };
            const now = Timestamp.now();
            const reservation = { paymentId: operationId, operationId, customerId: actor.uid, provider: 'YOCO', amountMinor: operation.package.priceMinor, currency: 'ZAR', status: 'PENDING', createdAt: existing?.createdAt ?? now, updatedAt: now };
            transaction.set(paymentRef, reservation);
            return { operation, payment: reservation, reuse: false };
        });
        if (eligibility.reuse)
        ...
        let checkout;
        try {
            checkout = await createYocoCheckout({ paymentId, operationId, amountMinor: eligibility.operation.package.priceMinor, currency: 'ZAR', customerId: actor.uid });
        }
        catch (error) {
            const now = Timestamp.now();
            await paymentRef.update({ status: 'FAILED', failureCategory: error instanceof YocoProviderConfigurationError ? 'PROVIDER_NOT_CONFIGURED' : 'PROVIDER_INITIATION_FAILED', failedAt: now, updatedAt: now });
            throw error;
        }
        const now = Timestamp.now();
        const payment = { paymentId, operationId, customerId: actor.uid, provider: 'YOCO', amountMinor: eligibility.operation.package.priceMinor, currency: 'ZAR', status: 'PENDING', providerCheckoutId: checkout.providerCheckoutId, checkoutUrl: checkout.checkoutUrl, createdAt: eligibility.payment?.createdAt ?? now, updatedAt: now };
        await db.runTransaction(async (transaction) => { const current = await transaction.get(operationRef); if (!current.exists)
            throw new HttpsError('not-found', 'Operation not found.'); const operation = current.data(); if (operation.customerId !== actor.uid || operation.status !== 'PAYMENT_PENDING' || operation.paymentSummary.status !== 'PENDING')
            throw new HttpsError('failed-precondition', 'Payment eligibility changed.'); transaction.set(paymentRef, payment); transaction.create(activityRef, { operationId, type: 'PAYMENT_INITIATED', timestamp: now, actorId: actor.uid, actorRole: 'CUSTOMER', fromStatus: 'PAYMENT_PENDING', toStatus: 'PAYMENT_PENDING', note: 'Secure payment checkout initiated.' }); });
        return { paymentId, checkoutUrl: checkout.checkoutUrl, status: 'PENDING' };
    }
    catch (error) {
