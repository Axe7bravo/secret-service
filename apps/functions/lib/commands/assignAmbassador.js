...
const eligible = (ambassador, operation) => ambassador.campusCodes.length === 0 || ambassador.campusCodes.includes(normalize(operation.recipient.campus));
export const assignAmbassador = onCall(async (request) => {
    ...
    await db.runTransaction(async (transaction) => { const [operationSnapshot, ambassadorSnapshot] = await Promise.all([transaction.get(operationRef), transaction.get(ambassadorRef)]); if (!operationSnapshot.exists)
