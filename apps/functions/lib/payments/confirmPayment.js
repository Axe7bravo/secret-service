...
export const confirmOperationPayment = async (event) => {
    ...
    return db.runTransaction(async (transaction) => {
        ...
        if (payment.status === 'PAID') {
            ...
            if (settledCheckoutId === event.providerCheckoutId && payment.providerPaymentId === event.providerPaymentId)
            ...
            if (attempt.status !== 'SUCCEEDED') {
                ...
                attempt.providerPaymentId = event.providerPaymentId;
                transaction.update(paymentRef, paymentHistoryFields(attempts));
            }
            return 'ADDITIONAL_SUCCESS';
        }
        if (!['PENDING', 'FAILED'].includes(payment.status))
            throw new HttpsError('failed-precondition', 'Payment is not eligible for provider-confirmed settlement.');
        ...
        if (operation.paymentSummary.status !== 'PENDING')
            throw new HttpsError('failed-precondition', 'Operation payment summary needs reconciliation.');
        validateTransition(operation.status, 'PAID', {});
        const now = Timestamp.now();
        const next = { ...operation, status: 'PAID', paymentSummary: { ...operation.paymentSummary, status: 'PAID', paidAt: now }, updatedAt: now };
        attempt.status = 'SUCCEEDED';
        attempt.providerPaymentId = event.providerPaymentId;
        const nextPayment = { ...payment, ...paymentHistoryFields(attempts), status: 'PAID', settledCheckoutId: event.providerCheckoutId, providerCheckoutId: event.providerCheckoutId, providerPaymentId: event.providerPaymentId, paidAt: now, updatedAt: now };
        ...
        if (payment.providerCheckoutId !== event.providerCheckoutId)
            delete nextPayment.checkoutUrl;
