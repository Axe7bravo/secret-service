...
export const createOperationPayment=onCall<Input>({region:'us-central1',secrets:[yocoSecretKey]},async request=>{
  ...
  try{
    ...
    const eligibility=await db.runTransaction(async transaction=>{
      ...
      if(existing?.status==='PENDING'&&existing.checkoutUrl)return{operation,payment:existing,checkoutReady:true as const};
      ...
      const reservation:PaymentRecord={paymentId:operationId,operationId,customerId:actor.uid,provider:'YOCO',amountMinor:operation.package.priceMinor,currency:'ZAR',status:'PENDING',idempotencyKey,attemptNumber,createdAt:existing?.createdAt??now,updatedAt:now};
      transaction.set(paymentRef,reservation);
      return{operation,payment:reservation,idempotencyKey,checkoutReady:false as const};
    });
    ...
    try{
      ...
      await db.runTransaction(async transaction=>{
        ...
        if(current.status==='PENDING'&&current.idempotencyKey===eligibility.idempotencyKey&&!current.providerCheckoutId)transaction.update(paymentRef,{status:'FAILED',failureCategory:error instanceof YocoProviderConfigurationError?'PROVIDER_NOT_CONFIGURED':'PROVIDER_INITIATION_FAILED',failedAt:now,updatedAt:now});
      });
    ...
    const now=Timestamp.now();
    const payment:PaymentRecord={...eligibility.payment,providerCheckoutId:checkout.providerCheckoutId,checkoutUrl:checkout.checkoutUrl,...(checkout.processingMode?{processingMode:checkout.processingMode}:{}),...(checkout.providerPaymentId?{providerPaymentId:checkout.providerPaymentId}:{}),updatedAt:now};
    const storedCheckout=await db.runTransaction(async transaction=>{
      ...
      if(operation.customerId!==actor.uid||operation.status!=='PAYMENT_PENDING'||operation.paymentSummary.status!=='PENDING')throw new HttpsError('failed-precondition','Payment eligibility changed.');
      if(currentPayment.exists){
        const current=currentPayment.data() as PaymentRecord;
        if(current.status==='PENDING'&&current.providerCheckoutId&&current.checkoutUrl)return{paymentId:current.paymentId,checkoutUrl:current.checkoutUrl};
        if(current.idempotencyKey!==payment.idempotencyKey)throw new HttpsError('aborted','A newer payment attempt has started.');
      }
