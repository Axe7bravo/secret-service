...
export const savePackage=onCall<unknown>(async request=>{
  ...
  try{
    const input=parse(request.data);const db=getAdminFirestore();const packageId=input.packageId??input.code;const ref=db.collection('packages').doc(packageId);const now=Timestamp.now();
    await db.runTransaction(async transaction=>{const snapshot=await transaction.get(ref);if(input.packageId&&!snapshot.exists)throw new HttpsError('not-found','Package no longer exists.');if(!input.packageId&&snapshot.exists)throw new HttpsError('already-exists','A package with that code already exists.');const existing=snapshot.exists?snapshot.data() as PackageRecord:undefined;if(existing&&existing.code!==input.code)throw new HttpsError('failed-precondition','Package code is a stable identifier and cannot be changed.');const record:PackageRecord={packageId,code:input.code,name:input.name,shortDescription:input.shortDescription,...(input.description?{description:input.description}:{}),priceMinor:input.priceMinor,currency:'ZAR',active:input.active,displayOrder:input.displayOrder,createdAt:existing?.createdAt??now,updatedAt:now};transaction.set(ref,record)});
    return{packageId};
