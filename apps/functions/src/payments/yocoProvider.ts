...
const requiredString=(record:Record<string,unknown>,key:string):string=>{
  ...
  if(typeof value!=='string'||!value.trim())throw new YocoProviderResponseError();
  return value;
};
const safeProviderError=(body:string):Record<string,string>=>{
  try{
...
export const createYocoCheckout:YocoCheckoutProvider=async request=>{
  ...
  if(!/^sk_(test|live)_/.test(secretKey))throw new YocoProviderConfigurationError('YOCO_SECRET_KEY is missing or has an invalid key prefix.');
  ...
  let decoded:unknown;
  try{decoded=JSON.parse(responseBody)}catch{
  ...
  if(!body)throw new YocoProviderResponseError();
  const providerCheckoutId=requiredString(body,'id');
  const checkoutUrl=requiredString(body,'redirectUrl');
  if(body.amount!==request.amountMinor||body.currency!==request.currency)throw new YocoProviderResponseError();
  try{if(new URL(checkoutUrl).protocol!=='https:')throw new YocoProviderResponseError()}catch(error){if(error instanceof YocoProviderResponseError)throw error;throw new YocoProviderResponseError()}
  const result:YocoCheckoutResult={providerCheckoutId,checkoutUrl};
  if(typeof body.processingMode==='string')result.processingMode=body.processingMode;
  if(typeof body.paymentId==='string'&&body.paymentId)result.providerPaymentId=body.paymentId;
  return result;
};
