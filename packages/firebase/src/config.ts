...
export const firebaseConfigurationError = missing.length
  ? `Firebase is not configured. Missing: ${missing.join(', ')}. Copy .env.example to a local .env file and provide the Firebase web configuration.`
  : null;
...
export const getFirebaseWebConfig = (): FirebaseWebConfig => {
  ...
  return entries as FirebaseWebConfig;
};
